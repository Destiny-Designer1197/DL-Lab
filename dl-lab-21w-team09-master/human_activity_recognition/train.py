...
class Trainer(object):
    def __init__(self, model, ds_train, ds_val, run_paths,total_steps, log_interval,ckpt_interval,n_classes):
        log_dir = 'human_activity_recognition/logs/gradient_tape/'
    ...
    def train(self):
        with tf.profiler.experimental.Profile('human_actvity_recognition/logs/gradient_tape/train/'):
            for idx, (images, labels) in enumerate(self.ds_train):

                step = int(self.epoch_counter.assign_add(1))
                ...
                if step % self.log_interval == 0:
                    ...
                    with self.train_summary_writer.as_default():
                        ...
                        tf.summary.scalar('Train Accuracy',self.train_accuracy.result() * 100, step = step)
                        tf.summary.image('Train ConfusionMatrix',self.train_confusion_matrix_metrics.plot_conf_mat(issaved=False),step=step)

                    with self.val_summary_writer.as_default():
                        tf.summary.scalar('Val Loss',self.test_loss.result() , step = step)
                        tf.summary.scalar('Val Accuracy',self.test_accuracy.result() * 100, step = step)
                    ...
                    # wait += 1
                    # if self.test_accuracy > best:
                    #     best = self.test_accuracy
                    #     wait = 0
                    # if wait >= patience:
                    #     break
                ...
                if step % self.total_steps == 0:
                    ...
                    #     logging.error(f"Failed to save model")
